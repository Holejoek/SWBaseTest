...
open class BaseTarget<T1: Encodable, T2: Decodable>: TargetType {
    
    open var path: String = ""
    public let baseURL: URL = URL(string: "https://backend.mobilization2021.tk/")!
...
//    let baseURL: URL = PlistManager<BaseURL>().value.url
    open var method: Moya.Method = .get
    public let sampleData: Data = Data()
    ...
    public init(model: RequestModel<T1, T2>, method: Moya.Method = .get, multipart: [MultipartFormData]? = nil) {
        self.model = model
        self.method = method
